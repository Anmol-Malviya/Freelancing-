...
import { Upload, X, Plus, Loader2, AlertCircle, FileArchive, CheckCircle2, CloudUpload, ImageIcon, Link, Github, Save } from 'lucide-react';
...
export default function EditProjectPage() {
    ...
    // File upload state (optional for edit)
    ...
    if (!user) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
                ...
                <p className="text-gray-400">Please <a href="/auth/login" className="text-brand-400">sign in</a> to edit projects.</p>
            </div>
        );
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (uploadedFile) {
                updatePayload.s3_file_key = uploadedFile.file_id;
                updatePayload.file_url = uploadedFile.file_url;
            }

            await projectsApi.update(id as string, updatePayload);
            toast.success('Project updated! 🎉');
            router.push(`/projects/${id}`);
        } catch (err: any) {
            const detail = err?.response?.data?.detail;
            const msg = typeof detail === 'string' ? detail : Array.isArray(detail) ? detail.map((d: any) => d.msg).join(', ') : 'Update failed';
            toast.error(msg);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
            <div className="mb-8">
                <h1 className="section-title text-3xl">Edit Project</h1>
                <p className="section-sub">Update your project details</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="glass p-6 space-y-4">
                    <h2 className="font-semibold text-white">Replace Project File <span className="font-normal text-sm text-gray-400">(Optional)</span></h2>

                    <p className="text-sm text-gray-400 mb-4">Leave this blank to keep your currently published ZIP file.</p>
                                ...
                                <div className="flex flex-col items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-gray-300 font-medium">
                                            {dragActive ? 'Drop your file here' : 'Drag & drop a new project ZIP'}
                                        </p>
                        ...
                        <div className="flex items-center gap-4 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-emerald-400 text-sm flex items-center gap-1">
                                    <CheckCircle2 size={14} /> New File Uploaded • {formatSize(uploadedFile.file_size)}
                                </p>
                            </div>
                            <button
                                type="button"
